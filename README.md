...
7. Go to `Control Panel\Clock and Region`
8. Click on `Define date and time`
9.  Go to tab `Internet clock` and press `Change configurations`
10. On Server type the ip of adserver (192.168.0.10 in this scenario) and click `sync now`
11. Click on `Ok` and then on `Ok`
12. Go to windows explorer, right click on the computer and then click `Properties`
13. Click on `Change settings`
14. Click on `change` and them define:
